function out = labyrinthine_isobrutal_COMSOL
%
% labyrinthine_v_COMSOL.m
%
% Model exported on Aug 15 2016, 16:20 by COMSOL 5.2.0.220.
...
model.param.set('a3', 'b3-b2-3/2*d');
...
model.geom('geom1').feature('r1').set('size', {'d' 'H-a3-3/2*d'});
model.geom('geom1').feature('r1').set('base', 'center');
model.geom('geom1').feature('r1').set('contributeto', 'csel1');
model.geom('geom1').feature('r1').set('pos', {'0' 'H/2+a3/2+1/4*d'});
...
model.geom('geom1').feature('r3').set('size', {'d' 'H-a1-3/2*d'});
model.geom('geom1').feature('r3').set('base', 'center');
model.geom('geom1').feature('r3').set('contributeto', 'csel1');
model.geom('geom1').feature('r3').set('pos', {'b1-b3' 'H/2+a1/2+1/4*d'});
...
model.cpl('intop2').selection.set([96]);
...
model.physics('acpr').feature('pwr1').selection.set([1 96]);
